    ...
    try {
      ...
      const mappedJobs: Job[] = rawJobs.map((apiJob: any) => {
        ...
        if (apiJob.salaryMin && apiJob.salaryMax && apiJob.salaryMin !== apiJob.salaryMax) {
          ...
          salaryStr = `R$ ${(apiJob.salaryMin / 1000).toFixed(0)}k (Exact)`;
        } else if (apiJob.salaryMin) {
          salaryStr = `Starting from R$ ${(apiJob.salaryMin / 1000).toFixed(0)}k`;
        } else if (apiJob.exactSalary) {
          salaryStr = `R$ ${(apiJob.exactSalary / 1000).toFixed(0)}k (Exact)`;
        }
